...
async function main() {
  ...
  let syncDailyQuestion, syncDailyLanguage;
  if (TEST_MODE) {
    if (
      !config.GITHUB_TOKEN ||
      !config.GITHUB_REPO ||
    ...
    syncDailyQuestion = config.SYNC_DAILY_QUESTION === true || config.SYNC_DAILY_QUESTION === 'true';
    syncDailyLanguage = config.SYNC_DAILY_LANGUAGE || 'python3';
  } else {
    ...
    syncDailyQuestion = core.getInput("sync-daily-question") === 'true';
    syncDailyLanguage = core.getInput("sync-daily-language") || 'python3';
  }
