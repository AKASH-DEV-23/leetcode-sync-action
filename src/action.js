...
async function getDailyQuestion(leetcodeSession, csrfToken) {
  log(`Getting daily question...`);
  const headers = graphqlHeaders(leetcodeSession, csrfToken);
  const graphql = JSON.stringify({
  ...
  try {
    ...
    log(`Failed to get daily question: ${error}`);
    return null;
  }
}

async function submitToLeetCode(titleSlug, questionId, lang, code, leetcodeSession, csrfToken) {
  log(`Submitting boilerplate code to LeetCode...`);
  const headers = {
    ...graphqlHeaders(leetcodeSession, csrfToken),
  ...
  try {
    ...
    log(`Successfully submitted to LeetCode, submission ID: ${response.data.submission_id}`);
    return response.data;
  } catch (error) {
    log(`Failed to submit to LeetCode: ${error}`);
    return null;
  }
}
...
async function sync(inputs) {
  ...
  for (i = submissions.length - 1; i >= 0; i--) {
    ...
    const questionObj = await getQuestionData(
    ...
    const questionData = questionObj.content;
  ...
  if (syncDailyQuestion) {
    const daily = await getDailyQuestion(leetcodeSession, leetcodeCSRFToken);
    if (daily) {
      const name = normalizeName(daily.title);
      let alreadySyncedThisRun = false;
      for (const sub of submissions) {
        if (normalizeName(sub.title) === name) {
          alreadySyncedThisRun = true;
          break;
        }
      }

      if (!alreadySyncedThisRun) {
        const questionObj = await getQuestionData(
          daily.titleSlug,
          leetcodeSession,
          leetcodeCSRFToken
        ...
        if (questionObj !== null) {
          const questionData = questionObj.content;
          const lang = syncDailyLanguage || 'python3';
          let defaultCode = "";
          if (questionObj.codeSnippets) {
            const snippet = questionObj.codeSnippets.find(s => s.langSlug === lang);
            if (snippet) {
              defaultCode = snippet.code;
            }
          }
          
          await submitToLeetCode(
            daily.titleSlug,
